...
public final class EntitySenses1_18_R1 implements EntitySenses {
    ...
    public @NotNull List<Sensor<?>> getSensors() {
        return sensorsHandle.values()
                .stream()
                .map(ChipUtil1_18_R1::fromNMS)
                .collect(Collectors.toList());
    }

    @Override
    public void addSensor(@NotNull Sensor<?> sensor) throws IllegalArgumentException {
        if (!new ChipUtil1_18_R1().existsSensor(sensor.getKey())) throw new IllegalArgumentException("Unregistered Sensor: " + sensor.getKey());

        sensorsHandle.put(ChipUtil1_18_R1.toNMSType(sensor), ChipUtil1_18_R1.toNMS(sensor));
        save();
    }
    ...
    public void removeSensor(@NotNull NamespacedKey key) {
        if (!new ChipUtil1_18_R1().existsSensor(key)) throw new IllegalArgumentException("Unregistered Sensor: " + key);

        ResourceLocation keyH = ChipUtil1_18_R1.toNMS(key);
        Iterator<Map.Entry<SensorType<?>, net.minecraft.world.entity.ai.sensing.Sensor<?>>> it = sensorsHandle.entrySet().iterator();
