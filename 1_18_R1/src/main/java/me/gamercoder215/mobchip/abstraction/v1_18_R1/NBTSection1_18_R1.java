...
public class NBTSection1_18_R1 implements NBTSection {
    ...
    public NBTSection1_18_R1(CompoundTag tag, Runnable saveFunc, String path) {
        this.tag = tag;
        this.saveFunc = saveFunc;
        this.currentPath = path;
    }

    public NBTSection1_18_R1(Mob m) {
        this.tag = new CompoundTag();
        this.currentPath = "";
        ChipUtil1_18_R1.toNMS(m).saveWithoutId(tag);
        this.saveFunc = () -> ChipUtil1_18_R1.toNMS(m).load(tag);
    }
    ...
    static Object deserialize(Tag v) {
        if (v instanceof ListTag list) {
            ...
            list.stream().map(NBTSection1_18_R1::deserialize).forEach(l::add);
            return l.toArray();
        }
