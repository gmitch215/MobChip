...
public final class EntityBody1_18_R2 implements EntityBody {
    private final net.minecraft.world.entity.Mob nmsMob;

    public EntityBody1_18_R2(Mob nmsMob) {
        this.nmsMob = ChipUtil1_18_R2.toNMS(nmsMob);
    }
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        return switch (nmsMob.interact(ChipUtil1_18_R2.toNMS(p), h)) {
    ...
    public boolean isInvisibleTo(@Nullable Player p) {
        return nmsMob.isInvisibleTo(ChipUtil1_18_R2.toNMS(p));
    }
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE -> {
                ...
                for (Player p : ChipUtil1_18_R2.fromNMS(nmsMob).getWorld().getPlayers())
                    ChipUtil1_18_R2.toNMS(p).connection.send(pkt);
            }
    ...
    public boolean isInvulnerableTo(EntityDamageEvent.@Nullable DamageCause cause) {
        return nmsMob.isInvulnerableTo(ChipUtil1_18_R2.toNMS(cause));
    }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return EntitySelector.pushableBy(ChipUtil1_18_R2.toNMS(entity)).test(ChipUtil1_18_R2.toNMS(entity));
    }
