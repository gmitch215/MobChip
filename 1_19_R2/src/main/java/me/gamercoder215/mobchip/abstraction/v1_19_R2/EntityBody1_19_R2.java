...
public final class EntityBody1_19_R2 implements EntityBody {
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        return switch (nmsMob.interact(ChipUtil1_19_R2.toNMS(p), h)) {
    ...
    public boolean isInvisibleTo(@Nullable Player p) {
        return nmsMob.isInvisibleTo(ChipUtil1_19_R2.toNMS(p));
    }
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE -> {
                ...
                for (Player p : ChipUtil1_19_R2.fromNMS(nmsMob).getWorld().getPlayers())
                    ChipUtil1_19_R2.toNMS(p).connection.send(pkt);
            }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return EntitySelector.pushableBy(ChipUtil1_19_R2.toNMS(entity)).test(ChipUtil1_19_R2.toNMS(entity));
    }
    ...
    public void eat(@NotNull ItemStack item) {
        nmsMob.eat(ChipUtil1_19_R2.toNMS(m.getWorld()), ChipUtil1_19_R2.toNMS(item));
    }
}
