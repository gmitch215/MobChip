...
public class ChipUtil1_13_R2 implements ChipUtil {
    ...
    private static Entity fromNMS(net.minecraft.server.v1_13_R2.Entity en) {
        return en.getBukkitEntity();
    }

    private static me.gamercoder215.mobchip.combat.CombatEntry fromNMS(Mob m, net.minecraft.server.v1_13_R2.CombatEntry en) {
        int time = 0;
        float health = 0;
    ...
    private static net.minecraft.server.v1_13_R2.CombatEntry toNMS(me.gamercoder215.mobchip.combat.CombatEntry en) {
    ...
    private static class EntityCombatTracker1_13_R2 implements EntityCombatTracker {

        private final CombatTracker handle;
        private final Mob m;

        EntityCombatTracker1_13_R2(Mob m) {
            this.m = m;
        ...
        public @NotNull List<me.gamercoder215.mobchip.combat.CombatEntry> getCombatEntries() {
            ...
            try {
                ...
                f.setAccessible(true);
                ((List<net.minecraft.server.v1_13_R2.CombatEntry>) f.get(handle)).stream().map(en -> fromNMS(m, en)).forEach(entries::add);
            } catch (Exception e) {
                Bukkit.getLogger().severe(e.getClass().getSimpleName());
                Bukkit.getLogger().severe(e.getMessage());
                for (StackTraceElement s : e.getStackTrace()) Bukkit.getLogger().severe(s.toString());
    ...
    private static final class DragonPhase1_13_R2 implements DragonPhase {

        private final EnderDragon dragon;
        private final IDragonController handle;

        DragonPhase1_13_R2(EnderDragon dragon, IDragonController handle) {
            this.dragon = dragon;
            this.handle = handle;
        }
    ...
    private static NBTBase serialize(Object v) {
        ...
        switch (v.getClass().getSimpleName().toLowerCase()) {
            case "short": return new NBTTagShort((short) v);
            case "float": return new NBTTagFloat((float) v);
            case "long": return new NBTTagLong((long) v);
            case "byte": return new NBTTagByte((byte) v);
            case "integer": case "int": return new NBTTagInt((int) v);
            case "double": return new NBTTagDouble((double) v);
            case "uuid": {
                UUID uid = (UUID) v;
    ...
    private static Object deserialize(NBTBase v) {
        if (v instanceof NBTTagList) {
            ...
            list.stream().map(ChipUtil1_13_R2::deserialize).forEach(l::add);
            return l.toArray();
        }
        ...
        switch (v.getTypeId()) {
            ...
            case 7: return ((NBTTagByteArray) v).c();
            default: return v.asString();
        }
    }

    private static class NBTSection1_13_R2 implements NBTSection {
        ...
        NBTSection1_13_R2(NBTTagCompound tag, Runnable saveFunc, String path) {
            this.tag = tag;
            this.saveFunc = saveFunc;
            this.currentPath = path;
        }

        NBTSection1_13_R2(Mob m) {
        ...
        public @Nullable NBTSection getSection(@Nullable String key) {
            return tag.get(key) == null ? null : new NBTSection1_13_R2(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
            return tag.get(key) == null ? def : new NBTSection1_13_R2(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @NotNull NBTSection getOrCreateSection(@NotNull String key) throws IllegalArgumentException {
            return getSection(key, new NBTSection1_13_R2(new NBTTagCompound(), this::save, currentPath + "." + key));
        }
