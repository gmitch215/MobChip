...
public final class EntityBody1_13_R2 implements EntityBody {
    private final EntityInsentient nmsMob;

    public EntityBody1_13_R2(Mob nmsMob) {
        this.nmsMob = ChipUtil1_13_R2.toNMS(nmsMob);
    }
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        if (nmsMob.b(ChipUtil1_13_R2.toNMS(p), h)) return InteractionResult.SUCCESS;
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE: {
                ...
                for (Player p : ChipUtil1_13_R2.fromNMS(nmsMob).getWorld().getPlayers())
                    ChipUtil1_13_R2.toNMS(p).playerConnection.sendPacket(pkt);
                break;
            }
    ...
    public boolean isInvulnerableTo(EntityDamageEvent.@Nullable DamageCause cause) {
        return nmsMob.isInvulnerable(ChipUtil1_13_R2.toNMS(cause));
    }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return IEntitySelector.a(ChipUtil1_13_R2.toNMS(entity)).test(ChipUtil1_13_R2.toNMS(entity));
    }
