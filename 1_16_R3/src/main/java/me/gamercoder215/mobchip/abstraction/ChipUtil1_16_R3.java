...
public class ChipUtil1_16_R3 implements ChipUtil {
    ...
    private static Entity fromNMS(net.minecraft.server.v1_16_R3.Entity en) {
        return en.getBukkitEntity();
    }

    private static CombatEntry fromNMS(Mob m, net.minecraft.server.v1_16_R3.CombatEntry en) {
        int time = 0;
        float health = 0;
    ...
    private static net.minecraft.server.v1_16_R3.CombatEntry toNMS(me.gamercoder215.mobchip.combat.CombatEntry en) {
    ...
    private static class EntityCombatTracker1_16_R3 implements EntityCombatTracker {

        private final CombatTracker handle;
        private final Mob m;

        EntityCombatTracker1_16_R3(Mob m) {
            this.m = m;
        ...
        public @NotNull List<CombatEntry> getCombatEntries() {
            ...
            try {
                ...
                f.setAccessible(true);
                ((List<net.minecraft.server.v1_16_R3.CombatEntry>) f.get(handle)).stream().map(en -> fromNMS(m, en)).forEach(entries::add);
            } catch (Exception e) {
                Bukkit.getLogger().severe(e.getClass().getSimpleName());
                Bukkit.getLogger().severe(e.getMessage());
                for (StackTraceElement s : e.getStackTrace()) Bukkit.getLogger().severe(s.toString());
    ...
    private static final class DragonPhase1_16_R3 implements DragonPhase {

        private final EnderDragon dragon;
        private final IDragonController handle;

        DragonPhase1_16_R3(EnderDragon dragon, IDragonController handle) {
            this.dragon = dragon;
            this.handle = handle;
        }
    ...
    private static Object deserialize(NBTBase v) {
        if (v instanceof NBTTagList) {
            ...
            list.stream().map(ChipUtil1_16_R3::deserialize).forEach(l::add);
            return l.toArray();
        }
    ...
    private static class NBTSection1_16_R3 implements NBTSection {
        ...
        NBTSection1_16_R3(NBTTagCompound tag, Runnable saveFunc, String path) {
            this.tag = tag;
            this.saveFunc = saveFunc;
            this.currentPath = path;
        }

        NBTSection1_16_R3(Mob m) {
        ...
        public @Nullable NBTSection getSection(@Nullable String key) {
            return tag.get(key) == null ? null : new NBTSection1_16_R3(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
            return tag.get(key) == null ? def : new NBTSection1_16_R3(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @NotNull NBTSection getOrCreateSection(@NotNull String key) throws IllegalArgumentException {
            return getSection(key, new NBTSection1_16_R3(new NBTTagCompound(), this::save, currentPath + "." + key));
        }
    ...
    private static final class EntityNBT1_16_R3 extends NBTSection1_16_R3 implements EntityNBT {
