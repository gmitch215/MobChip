...
public final class PathfinderUseItem extends Pathfinder implements Conditional<Mob> {
    ...
    public PathfinderUseItem(@NotNull UseItemGoal<?> g) {
        ...
        this.item = MobChipUtil.convert(Pathfinder.getField(g, "b", net.minecraft.world.item.ItemStack.class));
        this.finishSound = MobChipUtil.convert(Pathfinder.getField(g, "d", SoundEvent.class));
    ...
    public UseItemGoal<?> getHandle() {
        return new UseItemGoal<>(nmsEntity, MobChipUtil.convert(item), MobChipUtil.convert(finishSound), m -> requirements.test(MobChipUtil.convert(m)));
    }
