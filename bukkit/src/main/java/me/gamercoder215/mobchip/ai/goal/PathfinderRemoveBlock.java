...
public final class PathfinderRemoveBlock extends Pathfinder implements SpeedModifier {
    ...
    public PathfinderRemoveBlock(@NotNull RemoveBlockGoal g) {
        ...
        try {
            ...
            net.minecraft.world.level.block.Block b = getField(g, "g", net.minecraft.world.level.block.Block.class);
    ...
    public RemoveBlockGoal getHandle() {
        return new RemoveBlockGoal(MobChipUtil.convert(entity.getWorld()).getBlockState(new BlockPos(toRemove.getX(), toRemove.getY(), toRemove.getZ())).getBlock(), (PathfinderMob) nmsEntity, speedMod, (int) Math.floor(entity.getLocation().distance(toRemove.getLocation())) + 1);
    }
