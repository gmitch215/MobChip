...
public final class PathfinderNearestAttackableTargetRaider<T extends LivingEntity> extends PathfinderNearestAttackableTarget<T> {
    ...
    public NearestAttackableWitchTargetGoal<?> getHandle() {
        return new NearestAttackableWitchTargetGoal<>((net.minecraft.world.entity.raid.Raider) nmsEntity, MobChipUtil.toLivingNMSClass(this.getFilter()), this.getInterval(), this.mustSee(), this.mustReach(), m -> this.getCondition().test(MobChipUtil.convert(m)));
    }
