...
public final class PathfinderHurtByTarget extends TargetPathfinder {
    ...
    public PathfinderHurtByTarget(@NotNull HurtByTargetGoal g) {
        ...
        Class<?>[] nmsClasses = getField(g, "i", Class[].class);
        for (Class<?> clazz : nmsClasses) {
            for (EntityType type : EntityType.values()) {
                try {
                    if (MobChipUtil.toBukkitClass(Entity.class, clazz.asSubclass(net.minecraft.world.entity.Entity.class)).equals(type.getEntityClass())) t.add(type);
    ...
    public HurtByTargetGoal getHandle() {
        ...
        for (EntityType t : ignoring) classes.add(MobChipUtil.toNMSClass(t.getEntityClass()));
