...
public final class ChipUtil1_17_R1 implements ChipUtil {
    ...
    private static Object deserialize(NBTBase v) {
        if (v instanceof NBTTagList list) {
            ...
            list.stream().map(ChipUtil1_17_R1::deserialize).forEach(l::add);
            return l.toArray();
        }
    ...
    private static class NBTSection1_17_R1 implements NBTSection {
        ...
        NBTSection1_17_R1(NBTTagCompound tag, Runnable saveFunc, String path) {
            this.tag = tag;
            this.saveFunc = saveFunc;
            this.currentPath = path;
        }

        NBTSection1_17_R1(Mob m) {
        ...
        public @Nullable NBTSection getSection(@Nullable String key) {
            return tag.get(key) == null ? null : new NBTSection1_17_R1(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
            return tag.get(key) == null ? def : new NBTSection1_17_R1(tag.getCompound(key), this::save, currentPath + "." + key);
        }
