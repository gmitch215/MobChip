...
final class EntityScheduleManager1_19_R1 implements EntityScheduleManager {
    ...
    public @Nullable Schedule getCurrentSchedule() {
        return ChipUtil1_19_R1.fromNMS(nmsMob.getBrain().getSchedule());
    }

    @Override
    public void setSchedule(@NotNull Schedule s) {
        nmsMob.getBrain().setSchedule(ChipUtil1_19_R1.toNMS(s));
    }
    ...
    public void setDefaultActivity(@NotNull Activity a) {
        nmsMob.getBrain().setDefaultActivity(ChipUtil1_19_R1.toNMS(a));
    }
    ...
    public void setRunningActivity(@NotNull Activity a) {
        nmsMob.getBrain().setActiveActivityIfPossible(ChipUtil1_19_R1.toNMS(a));
    }

    @Override
    public @Nullable Activity getRunningActivity() {
        return nmsMob.getBrain().getActiveNonCoreActivity().isPresent() ? ChipUtil1_19_R1.fromNMS(nmsMob.getBrain().getActiveNonCoreActivity().get()) : null;
    }

    @Override
    public boolean isRunning(@NotNull Activity a) {
        return nmsMob.getBrain().isActive(ChipUtil1_19_R1.toNMS(a));
    }
    ...
    public Consumer<Mob> put(@NotNull Activity key, Consumer<Mob> value) {
        nmsMob.getBrain().addActivity(ChipUtil1_19_R1.toNMS(key), 0, ImmutableList.of(ChipUtil1_19_R1.toNMS(value)));
        return value;
    }
