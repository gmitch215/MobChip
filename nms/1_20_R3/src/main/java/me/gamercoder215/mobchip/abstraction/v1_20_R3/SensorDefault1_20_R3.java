...
final class SensorDefault1_20_R3 implements Sensor<LivingEntity> {
    ...
    public @NotNull List<Memory<?>> required() {
        return handle.requires().stream().map(ChipUtil1_20_R3::fromNMS).collect(Collectors.toList());
    }
    ...
    public void run(@NotNull World w, LivingEntity entity) {
        try {
            ...
            doTick.invoke(handle, ChipUtil1_20_R3.toNMS(w), ChipUtil1_20_R3.toNMS(entity));
        } catch (ReflectiveOperationException e) {
            Bukkit.getLogger().severe(e.getMessage());
            for (StackTraceElement st : e.getStackTrace()) Bukkit.getLogger().severe(st.toString());
        }
    ...
    public NamespacedKey getKey() {
        ...
        BuiltInRegistries.SENSOR_TYPE.stream()
                ...
                .findFirst()
                .ifPresent(s -> key.set(ChipUtil1_20_R3.fromNMS(BuiltInRegistries.SENSOR_TYPE.getKey(s))));
