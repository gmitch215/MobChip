...
class NBTSection1_20_R3 implements NBTSection {
    ...
    public NBTSection1_20_R3(CompoundTag tag, Runnable saveFunc, String path) {
        this.tag = tag;
        this.saveFunc = saveFunc;
        this.currentPath = path;
    }

    public NBTSection1_20_R3(Mob m) {
        this.tag = new CompoundTag();
        this.currentPath = "";
        ChipUtil1_20_R3.toNMS(m).saveWithoutId(tag);
        this.saveFunc = () -> ChipUtil1_20_R3.toNMS(m).load(tag);
    }
    ...
    static Object deserialize(Tag v) {
        if (v instanceof ListTag list) {
            ...
            list.stream().map(NBTSection1_20_R3::deserialize).forEach(l::add);
            return l.toArray();
        }
    ...
    public @Nullable NBTSection getSection(@Nullable String key) {
        return tag.get(key) == null ? null : new NBTSection1_20_R3(tag.getCompound(key), this::save, currentPath + "." + key);
    }

    @Override
    public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
        return tag.get(key) == null ? def : new NBTSection1_20_R3(tag.getCompound(key), this::save, currentPath + "." + key);
    }

    @Override
    public @NotNull NBTSection getOrCreateSection(@NotNull String key) throws IllegalArgumentException {
        return getSection(key, new NBTSection1_20_R3(new CompoundTag(), this::save, currentPath + "." + key));
    }
