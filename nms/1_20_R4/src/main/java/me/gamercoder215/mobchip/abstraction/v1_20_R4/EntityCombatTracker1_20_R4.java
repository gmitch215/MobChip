...
final class EntityCombatTracker1_20_R4 implements EntityCombatTracker {
    ...
    public EntityCombatTracker1_20_R4(Mob m) {
        this.m = m;
        this.handle = ChipUtil1_20_R4.toNMS(m).getCombatTracker();
    }
    ...
    public @Nullable CombatEntry getLatestEntry() {
        try {
            ...
            net.minecraft.world.damagesource.CombatEntry last = entries.getLast();

            return last == null ? null : ChipUtil1_20_R4.fromNMS(m, last);
    ...
    public @NotNull List<CombatEntry> getCombatEntries() {
        ...
        try {
            ...
            ((List<net.minecraft.world.damagesource.CombatEntry>) f.get(handle)).stream().map(en -> ChipUtil1_20_R4.fromNMS(m, en)).forEach(entries::add);
        } catch (Exception e) {
            ChipUtil.printStackTrace(e);
        }
        return entries;
    ...
    public boolean hasLastDamageCancelled() {
        return ChipUtil1_20_R4.toNMS(m).lastDamageCancelled;
    }
}
