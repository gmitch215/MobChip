...
final class EntityScheduleManager1_20_R4 implements EntityScheduleManager {
    ...
    public @Nullable Schedule getCurrentSchedule() {
        return ChipUtil1_20_R4.fromNMS(nmsMob.getBrain().getSchedule());
    }

    @Override
    public void setSchedule(@NotNull Schedule s) {
        nmsMob.getBrain().setSchedule(ChipUtil1_20_R4.toNMS(s));
    }

    @Override
    public @NotNull Set<Activity> getActiveActivities() {
        return nmsMob.getBrain().getActiveActivities().stream().map(ChipUtil1_20_R4::fromNMS).collect(Collectors.toSet());
    }

    @Override
    public void setDefaultActivity(@NotNull Activity a) {
        nmsMob.getBrain().setDefaultActivity(ChipUtil1_20_R4.toNMS(a));
    }
    ...
    public void setRunningActivity(@NotNull Activity a) {
        nmsMob.getBrain().setActiveActivityIfPossible(ChipUtil1_20_R4.toNMS(a));
    }

    @Override
    public @Nullable Activity getRunningActivity() {
        return nmsMob.getBrain().getActiveNonCoreActivity().isPresent() ? ChipUtil1_20_R4.fromNMS(nmsMob.getBrain().getActiveNonCoreActivity().get()) : null;
    }

    @Override
    public boolean isRunning(@NotNull Activity a) {
        return nmsMob.getBrain().isActive(ChipUtil1_20_R4.toNMS(a));
    }
    ...
    public Consumer<Mob> put(@NotNull Activity key, Consumer<Mob> value) {
        nmsMob.getBrain().addActivity(ChipUtil1_20_R4.toNMS(key), 0, ImmutableList.of(ChipUtil1_20_R4.toNMS(value)));
        return value;
    }
