...
final class EntitySenses1_20_R4 implements EntitySenses {
    ...
    public @NotNull List<Sensor<?>> getSensors() {
        return sensorsHandle.values()
                .stream()
                .map(ChipUtil1_20_R4::fromNMS)
                .collect(Collectors.toList());
    }

    @Override
    public void addSensor(@NotNull Sensor<?> sensor) throws IllegalArgumentException {
        if (!new ChipUtil1_20_R4().existsSensor(sensor.getKey())) throw new IllegalArgumentException("Unregistered Sensor: " + sensor.getKey());

        sensorsHandle.put(ChipUtil1_20_R4.toNMSType(sensor), ChipUtil1_20_R4.toNMS(sensor));
        save();
    }
    ...
    public void removeSensor(@NotNull NamespacedKey key) {
        if (!new ChipUtil1_20_R4().existsSensor(key)) throw new IllegalArgumentException("Unregistered Sensor: " + key);

        ResourceLocation keyH = ChipUtil1_20_R4.toNMS(key);
