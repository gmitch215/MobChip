...
final class EntityBody1_20_R1 implements EntityBody {
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        return switch (nmsMob.interact(ChipUtil1_20_R1.toNMS(p), h)) {
    ...
    public boolean isInvisibleTo(@Nullable Player p) {
        return nmsMob.isInvisibleTo(ChipUtil1_20_R1.toNMS(p));
    }
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE -> {
                ...
                for (Player p : ChipUtil1_20_R1.fromNMS(nmsMob).getWorld().getPlayers())
                    ChipUtil1_20_R1.toNMS(p).connection.send(pkt);
            }
    ...
    public boolean isInvulnerableTo(EntityDamageEvent.@Nullable DamageCause cause) {
        return nmsMob.isInvulnerableTo(ChipUtil1_20_R1.toNMS(cause, m));
    }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return EntitySelector.pushableBy(ChipUtil1_20_R1.toNMS(entity)).test(ChipUtil1_20_R1.toNMS(entity));
    }
    ...
    public void eat(@NotNull ItemStack item) {
        nmsMob.eat(ChipUtil1_20_R1.toNMS(m.getWorld()), ChipUtil1_20_R1.toNMS(item));
    }
