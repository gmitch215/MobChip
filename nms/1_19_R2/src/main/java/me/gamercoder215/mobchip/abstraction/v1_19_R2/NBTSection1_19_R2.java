...
class NBTSection1_19_R2 implements NBTSection {
    ...
    public NBTSection1_19_R2(CompoundTag tag, Runnable saveFunc, String path) {
        this.tag = tag;
        this.saveFunc = saveFunc;
        this.currentPath = path;
    }

    public NBTSection1_19_R2(Mob m) {
        this.tag = new CompoundTag();
        this.currentPath = "";
        ChipUtil1_19_R2.toNMS(m).saveWithoutId(tag);
        this.saveFunc = () -> ChipUtil1_19_R2.toNMS(m).load(tag);
    }
    ...
    static Object deserialize(Tag v) {
        if (v instanceof ListTag list) {
            ...
            list.stream().map(NBTSection1_19_R2::deserialize).forEach(l::add);
            return l.toArray();
        }
    ...
    public @Nullable NBTSection getSection(@Nullable String key) {
        return tag.get(key) == null ? null : new NBTSection1_19_R2(tag.getCompound(key), this::save, currentPath + "." + key);
    }

    @Override
    public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
        return tag.get(key) == null ? def : new NBTSection1_19_R2(tag.getCompound(key), this::save, currentPath + "." + key);
    }

    @Override
    public @NotNull NBTSection getOrCreateSection(@NotNull String key) throws IllegalArgumentException {
        return getSection(key, new NBTSection1_19_R2(new CompoundTag(), this::save, currentPath + "." + key));
    }
