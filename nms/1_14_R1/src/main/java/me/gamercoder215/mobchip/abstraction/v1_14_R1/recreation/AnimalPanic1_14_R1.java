package me.gamercoder215.mobchip.abstraction.v1_14_R1.recreation;
...
public class AnimalPanic1_14_R1 extends Behavior<EntityCreature> {

    private final float speedMultiplier;

    public AnimalPanic1_14_R1(float speedMod) {
    ...
    private Vec3D getPanicPos(EntityCreature c, WorldServer w) {
        if (c.isBurning()) {
            Optional<Vec3D> var2 = this.lookForWater(w, c).map(AnimalPanic1_14_R1::atBottomCenterOf);
    ...
    private static boolean isRestricted(EntityCreature c, int var1) {
        return c.dL() && c.dI().a(c.getPositionVector(), (double) (c.dJ() + (float) var1) + 1.0D);
    }
    ...
    public static boolean hasMalus(@NotNull EntityCreature c, @NotNull BlockPosition pos) {
        return c.a(new PathfinderNormal().a(c.world, pos.getX(), pos.getY(), pos.getZ())) != 0.0F;
    }
