...
final class EntityCombatTracker1_19_R3 implements EntityCombatTracker {
    ...
    public EntityCombatTracker1_19_R3(Mob m) {
        this.m = m;
        this.handle = ChipUtil1_19_R3.toNMS(m).getCombatTracker();
    }
    ...
    public @Nullable CombatEntry getLatestEntry() {
        return handle.getLastEntry() == null ? null : ChipUtil1_19_R3.fromNMS(m, handle.getLastEntry());
    }
    ...
    public @NotNull List<CombatEntry> getCombatEntries() {
        ...
        try {
            ...
            ((List<net.minecraft.world.damagesource.CombatEntry>) f.get(handle)).stream().map(en -> ChipUtil1_19_R3.fromNMS(m, en)).forEach(entries::add);
        } catch (Exception e) {
    ...
    public boolean hasLastDamageCancelled() {
        return ChipUtil1_19_R3.toNMS(m).lastDamageCancelled;
    }
}
