...
final class EntityCombatTracker1_20_R2 implements EntityCombatTracker {
    ...
    public EntityCombatTracker1_20_R2(Mob m) {
        this.m = m;
        this.handle = ChipUtil1_20_R2.toNMS(m).getCombatTracker();
    }
    ...
    public @Nullable CombatEntry getLatestEntry() {
        try {
            ...
            return last == null ? null : ChipUtil1_20_R2.fromNMS(m, last);
    ...
    public @NotNull List<CombatEntry> getCombatEntries() {
        ...
        try {
            ...
            ((List<net.minecraft.world.damagesource.CombatEntry>) f.get(handle)).stream().map(en -> ChipUtil1_20_R2.fromNMS(m, en)).forEach(entries::add);
        } catch (Exception e) {
            ChipUtil.printStackTrace(e);
        }
        return entries;
    ...
    public boolean hasLastDamageCancelled() {
        return ChipUtil1_20_R2.toNMS(m).lastDamageCancelled;
    }
}
