...
final class EntityBody1_20_R2 implements EntityBody {
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        return switch (nmsMob.interact(ChipUtil1_20_R2.toNMS(p), h)) {
    ...
    public boolean isInvisibleTo(@Nullable Player p) {
        return nmsMob.isInvisibleTo(ChipUtil1_20_R2.toNMS(p));
    }
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE -> {
                ...
                for (Player p : ChipUtil1_20_R2.fromNMS(nmsMob).getWorld().getPlayers())
                    ChipUtil1_20_R2.toNMS(p).connection.send(pkt);
            }
    ...
    public boolean isInBubbleColumn() {
        return nmsMob.getCommandSenderWorld().getBlockState(nmsMob.blockPosition()).is(Blocks.BUBBLE_COLUMN);
    }

    @Override
    public boolean isInvulnerableTo(EntityDamageEvent.@Nullable DamageCause cause) {
        return nmsMob.isInvulnerableTo(ChipUtil1_20_R2.toNMS(cause, m));
    }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return EntitySelector.pushableBy(ChipUtil1_20_R2.toNMS(entity)).test(ChipUtil1_20_R2.toNMS(entity));
    }
    ...
    public void setRiptideTicks(int ticks) {
        ...
        try {
            ...
            if (!nmsMob.getCommandSenderWorld().isClientSide) {
    ...
    public void eat(@NotNull ItemStack item) {
        nmsMob.eat(ChipUtil1_20_R2.toNMS(m.getWorld()), ChipUtil1_20_R2.toNMS(item));
    }
