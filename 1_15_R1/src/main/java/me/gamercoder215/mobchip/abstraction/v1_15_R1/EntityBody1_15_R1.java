...
public final class EntityBody1_15_R1 implements EntityBody {
    private final EntityInsentient nmsMob;
    private final Mob m;

    public EntityBody1_15_R1(Mob m) {
        this.m = m;
    ...
    public InteractionResult interact(@NotNull Player p, @Nullable InteractionHand hand) {
        ...
        if (nmsMob.b(ChipUtil1_15_R1.toNMS(p), h)) return InteractionResult.SUCCESS;
    ...
    public void setBodyRotation(float rotation) {
        nmsMob.aI = EntityBody.normalizeRotation(rotation);
    }
    ...
    public void setHeadRotation(float rotation) {
        nmsMob.aK = EntityBody.normalizeRotation(rotation);
    }
    ...
    public void playAnimation(@NotNull EntityAnimation anim) {
        switch (anim) {
            ...
            case CRITICAL_DAMAGE: {
                ...
                for (Player p : ChipUtil1_15_R1.fromNMS(nmsMob).getWorld().getPlayers())
    ...
    public boolean isInvulnerableTo(EntityDamageEvent.@Nullable DamageCause cause) {
        return nmsMob.isInvulnerable(ChipUtil1_15_R1.toNMS(cause));
    }
    ...
    public boolean isPushableBy(@Nullable Entity entity) {
        return IEntitySelector.a(ChipUtil1_15_R1.toNMS(entity)).test(ChipUtil1_15_R1.toNMS(entity));
    }
    ...
    public void eat(@NotNull ItemStack item) {
        nmsMob.a(ChipUtil1_15_R1.toNMS(m.getWorld()), ChipUtil1_15_R1.toNMS(item));
    }
