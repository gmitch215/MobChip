...
public final class EntitySenses1_15_R1 implements EntitySenses {

    private final Mob m;
    private final EntityInsentient nmsMob;

    private final Map<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>> sensorsHandle = new HashMap<>();

    public EntitySenses1_15_R1(Mob m) {
        this.m = m;
        this.nmsMob = ChipUtil1_15_R1.toNMS(m);

        try {
            Field sensorsF = BehaviorController.class.getDeclaredField("sensors");
            sensorsF.setAccessible(true);
            Map<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>> sensors = (Map<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>>) sensorsF.get(nmsMob.getBehaviorController());
    ...
    public @NotNull List<Sensor<?>> getSensors() {
        return sensorsHandle.values()
                .stream()
                .map(ChipUtil1_15_R1::fromNMS)
                .collect(Collectors.toList());
    }

    @Override
    public void addSensor(@NotNull Sensor<?> sensor) throws IllegalArgumentException {
        if (!new ChipUtil1_15_R1().existsSensor(sensor.getKey())) throw new IllegalArgumentException("Unregistered Sensor: " + sensor.getKey());

        sensorsHandle.put(ChipUtil1_15_R1.toNMSType(sensor), ChipUtil1_15_R1.toNMS(sensor));
        save();
    }
    ...
    public void removeSensor(@NotNull Sensor<?> sensor) {
        ...
        SensorType<?> type = ChipUtil1_15_R1.toNMSType(sensor);
        MinecraftKey key = IRegistry.SENSOR_TYPE.getKey(type);

        Iterator<Map.Entry<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>>> it = sensorsHandle.entrySet().iterator();

        while (it.hasNext()) {
            Map.Entry<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>> entry = it.next();
    ...
    public void removeSensor(@NotNull NamespacedKey key) {
        if (!new ChipUtil1_15_R1().existsSensor(key)) throw new IllegalArgumentException("Unregistered Sensor: " + key);

        MinecraftKey keyH = ChipUtil1_15_R1.toNMS(key);
        Iterator<Map.Entry<SensorType<?>, net.minecraft.server.v1_15_R1.Sensor<?>>> it = sensorsHandle.entrySet().iterator();
    ...
    public boolean hasSensor(@NotNull NamespacedKey key) {
        ...
        for (SensorType<?> t : sensorsHandle.keySet()) {
            ...
            if (ChipUtil1_15_R1.toNMS(key).equals(currentKey)) {
                b.set(true);
                break;
            }
        }
