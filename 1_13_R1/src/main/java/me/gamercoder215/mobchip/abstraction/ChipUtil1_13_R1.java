...
public final class ChipUtil1_13_R1 implements ChipUtil {
    ...
    private static Object deserialize(NBTBase v) {
        if (v instanceof NBTTagList) {
            ...
            list.stream().map(ChipUtil1_13_R1::deserialize).forEach(l::add);
            return l.toArray();
        }
        ...
        switch (v.getTypeId()) {
            case 1: return ((NBTTagByte) v).g();
            case 2: return ((NBTTagShort) v).f();
            case 3: return ((NBTTagInt) v).e();
            case 4: return ((NBTTagLong) v).d();
            case 5: return ((NBTTagFloat) v).i();
    ...
    private static class NBTSection1_13_R1 implements NBTSection {
        ...
        NBTSection1_13_R1(NBTTagCompound tag, Runnable saveFunc, String path) {
            this.tag = tag;
            this.saveFunc = saveFunc;
            this.currentPath = path;
        }

        NBTSection1_13_R1(Mob m) {
        ...
        public @Nullable NBTSection getSection(@Nullable String key) {
            return tag.get(key) == null ? null : new NBTSection1_13_R1(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @Nullable NBTSection getSection(@Nullable String key, @Nullable NBTSection def) {
            return tag.get(key) == null ? def : new NBTSection1_13_R1(tag.getCompound(key), this::save, currentPath + "." + key);
        }

        @Override
        public @NotNull NBTSection getOrCreateSection(@NotNull String key) throws IllegalArgumentException {
            return getSection(key, new NBTSection1_13_R1(new NBTTagCompound(), this::save, currentPath + "." + key));
        }
